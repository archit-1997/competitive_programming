...
//index stores the index of the given element in the array(or you can say the node)
//s and e store the starting and ending index of the array elements that the tree node will store
...
ll query(V &tree,ll index, ll s, ll e, ll qs, ll qe){
    ...
    //2. partial overlapp -> in this case, individually call for both the subtrees
    ...
    ll m=s+(e-s)/2;
    //note that we are only changing the index of the tree nodes and not of the query that we are sending down the tree
